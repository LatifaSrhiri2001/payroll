...
@section('content')
    @if (session('success'))
        <x-mary-alert class="bg-green-200" title="Success" description="{{ session('success') }}" icon="o-check-circle"
            dismissible />
    @endif
    
    {{-- <a href="{{ route('cotisations.create') }}" data-modal-target="default-modal" data-modal-toggle="default-modal"
    
    class="btn btn-primary">Ajouter une Cotisation</a> --}}
    <table class="table">
        <thead>
            <tr>
                <th>ID</th>
        ...
        <tbody>
            @foreach ($cotisations as $cotisation)
                <tr>
                    <td>{{ $cotisation->id }}</td>
                    <td>{{ $cotisation->employe_cin }}</td>
                    <td>{{ $cotisation->type_cotisation }}</td>
                    <td>{{ $cotisation->taux }}</td>
                    <td>{{ $cotisation->base_calcul }}</td>
                    <td>{{ $cotisation->montant }}</td>
                    {{-- <td>
                        <a href="{{ route('cotisations.edit', $cotisation->id) }}" class="btn btn-warning">Modifier</a>
                        <form id="delete-form-{{ $cotisation->id }}"
                            action="{{ route('cotisations.destroy', $cotisation->id) }}" method="POST"
                            style="display: inline;">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-danger"
                                onclick="return confirm('Êtes-vous sûr de vouloir supprimer cette cotisation?')">Supprimer</button>
                        </form>
                    </td> --}}
                </tr>
            @endforeach
          ...
          <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
              ...
              <div class="p-4 md:p-5 space-y-4">
                  <form action="{{ route('cotisations.store') }}" method="POST">
                      ...
                      <div>
                          <label for="taux" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Taux (%)</label>
                          <input type="number" id="taux" name="taux" required min="0" max="100" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="Entrez le taux">
                          @error('taux')
                              <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                          @enderror
                      </div>
